...
def main():
    ...
    ap.add_argument("--n_embd",type=int,default=256); ap.add_argument("--out",type=str,default=os.path.join(CK,"ternary.pt"))
    a=ap.parse_args(); dev="cpu"; torch.set_num_threads(4)
    tr,va=split("train"),split("val"); cfg=Config(block=a.block,n_embd=a.n_embd)
    ...
    for it in range(1,a.iters+1):
        ...
        if it%250==0 or it==1:
            ...
            if vp<best: best=vp; torch.save({"model":m.state_dict(),"cfg":vars(cfg),"val_ppl":vp},a.out)
